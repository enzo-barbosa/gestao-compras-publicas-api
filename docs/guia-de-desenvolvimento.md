...
- Validação: oxlint **0**, **Vitest 50/50**, `tsc -b && vite build` OK, backend **172/172**. Pendências: **Fase C** (opcionais: modal de confirmação no lugar de `window.confirm`; `usuarioNome` no DTO de empenho — usuário pediu análise com citações antes de decidir) e **regravação do GIF demo** após aprovação do visual (o `docs/img/demo.gif` antigo foi removido do working tree e será regravado do zero).
