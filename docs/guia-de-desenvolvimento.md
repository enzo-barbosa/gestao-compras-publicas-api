...
| 2026-09-07 | opencode/big-pickle | **Programa multitenancy — Fase 2 implementada e validada (aguardando commit)**: API de grupos (`POST/GET /api/organizacoes`, `GET/PUT /{id}`; quem cria vira ADMIN; lista com papel; não-membro → 403), membros (`GET/POST/PUT/DELETE /{id}/membros`; criador intocável; auto-alteração bloqueada), convites (`POST/GET/DELETE /{id}/convites` por e-mail XOR código com papel; `POST /api/convites/aceitar` por código; `POST /api/convites/aceitar-email`), `register` público de volta, `me` com lista de organizações. Segurança: regras específicas para `/api/organizacoes/**` (mutações exigem `X-Org-Id`→`ROLE_ADMIN`) e `/api/convites/**` (autenticado); CORS libera `X-Org-Id`. Nova exception `NaoMembroException`→403. Testes: `OrganizacaoServiceTest` (15) + `ConviteServiceTest` (17) + `OrganizacoesIntegrationTest` (8; register público, criar→ADMIN, me c/ orgs, adicionar membro, criador 409, aceites código/email, OPERADOR 403) + `AuthIntegrationTest` ajustado → **136/136** verdes. Pendência: commit da Fase 2. Próximo: Fase 3 (frontend). |
