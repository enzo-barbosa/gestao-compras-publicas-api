...
- [x] **Topo público**: novo componente `TopoPublico` (marca + Entrar/Criar conta) compartilhado por landing, login, cadastro, redefinir-senha, onboarding e seletor; CSS `tela-login` removido.
- [x] **Login/cadastro/redefinir-senha em 2 colunas**: `.tela-split` (gradiente azul) + `PainelMarca` (eyebrow + headline + bullets `✓`) à esquerda e formulário branco (`painel-form`, radius 24px) à direita; abaixo de 880px vira 1 coluna (painel de marca por cima).
...
- [x] A marca **"GCP Compras Públicas"** no topo esquerdo virou um link de "início": no `TopoPublico` (landing, login, cadastro, redefinir-senha, seleção de grupo) leva a **`/`** (homepage); na `Navbar` (todas as páginas logadas) leva a **`/app`** (Dashboard). CSS: `.marca` agora é âncora — `color: inherit` (mantém branco no navbar `/ #fff`) + `text-decoration: none` + hover de leve opacidade (0.85). `aria-label` no link ("Página inicial" / "Ir para o dashboard"). Sem rotas novas; Guardia/redirecionamentos intactos.
...
| 2026-09-17 | opencode/big-pickle | **Auditoria e atualização do README + deploy.md** (pedido do usuário: "confere se o README está 100% atualizado"). Reconciliação contra o código: (1) badge de cobertura `83.9%` → **`84.8%`** (alinhado ao `testes.md` e ao JaCoCo medido: 6816/8042 instruções; ramos 71,5%); (2) documentada a **recuperação de senha** por código de 6 dígitos via Resend (expira em 15 min; sem `RESEND_API_KEY` o `EmailService` loga o código — dev funciona sem e-mail) em Funcionalidades, na seção "Como rodar" e na tabela de endpoints (`POST /api/auth/esqueci-senha`, `POST /api/auth/redefinir-senha`); (3) tabela de endpoints corrigida/completada: `PUT /api/organizacoes/{id}` (antes atribuído a `/api/organizacoes`), `DELETE /api/organizacoes/{id}/membros/eu` (sair do grupo), linha de `/api/creditos-suplementares/**` e `GET /api/empenhos` + `GET /api/empenhos/{id}`; (4) `deploy.md` ganhou `RESEND_API_KEY` e `RESEND_EMAIL_FROM` na tabela de envs do Render. Verificados como corretos: `docs/img/demo.gif` existe, Flyway V1–V8, 180 backend / 51 front, 11 cenários Gherkin, smoke 52, JWT 8h e as 9 modalidades. Sem alteração de código. Commits sugeridos ao usuário: `docs: sync readme with password recovery and coverage` + `docs: document resend email env vars in deploy guide`. |
| 2026-09-17 | opencode/big-pickle | **Remoção da recuperação de senha por e-mail → redefinição confirmando a senha atual**. Decisão do usuário (não tem domínio próprio; montar Resend/Brevo/Gmail só para isso não compensa — fica para um projeto futuro com VPS/mecanismo próprio). Backend: apagados `EmailService`, `RecuperacaoSenhaService`, `RecuperacaoSenhaToken`, `RecuperacaoSenhaTokenRepository`, `EsqueciSenhaRequestDTO` e o endpoint `POST /api/auth/esqueci-senha`; `RedefinirSenhaRequestDTO` passou a `{email, senhaAtual, novaSenha}` e `AuthService.redefinirSenha` valida e-mail + `passwordEncoder.matches(senhaAtual)` com erro genérico "E-mail ou senha atual inválidos." (anti-enumeração), exige nova ≠ atual e faz bump de `versao_token` (revoga todas as sessões); migration **`V9__drop_recuperacao_senha.sql`** derruba a tabela `recuperacao_senha`; rota `esqueci-senha` removida do `RateLimitFilter` e `redefinir-senha` apertado para **5/15min**; props `app.email.resend.*`/`RESEND_*` removidas. Frontend: `EsqueciSenhaPage` → **`RedefinirSenhaPage`** (formulário único e-mail + senha atual + nova + confirmação) na rota `/redefinir-senha`, com o link do `LoginPage` ajustado. Testes: `RecuperacaoSenhaServiceTest` removido; `AuthIntegrationTest` com 3 casos (sucesso + login com a nova senha e 401 com a antiga, senha atual inválida 400, e-mail inexistente 400) → backend **172/172**, 0 falhas; cobertura **84,2% instruções / 69,8% ramos**; frontend oxlint **0** + Vitest **51/51** + `tsc -b && vite build` OK. Docs sincronizados (README, `testes.md`, `deploy.md` e este Log). Commits sugeridos ao usuário: `refactor(auth)!: validate current password on reset and drop email recovery`, `refactor(ui)!: reset password with current password form` e `docs: document password reset without email`. Próximo item combinado: reset de senha pelo **ADMIN do grupo** (opção C). |
| 2026-09-17 | opencode/big-pickle | **Plano C — reset de senha de membro pelo ADMIN do grupo** (fecha o gap de quem esqueceu a senha: os fluxos existentes exigem saber a senha atual). Backend: novo `dto/MembroSenhaRequestDTO` (`novaSenha` com `@Size(8..100)`) e `OrganizacaoService.redefinirSenhaMembro` (injeta `PasswordEncoder` no construtor) que reusa `org`/`exigirAdmin`/`membro` e um novo `protegerResetDeSenha` (mensagens específicas) — ADMIN do grupo ou SUPER_ADMIN, **sem redefinir a própria senha nem a do criador** (409), alvo precisa ser membro (404), nova = atual → 400 e bump de `versao_token` (revoga as sessões do alvo); rota `PUT /api/organizacoes/{id}/membros/{usuarioId}/senha` → **204** no `OrganizacaoController`, já coberta pelo matcher existente `PUT /api/organizacoes/**` → `hasAnyRole("ADMIN","SUPER_ADMIN")` (**sem mexer no `SecurityConfig`**). Frontend: novo `components/ModalRedefinirSenha.tsx` (nova senha + confirmação, validações de `validacao.ts`, Escape/focus) e ação "Redefinir senha" em `MembrosPage` (oculta para o próprio usuário; avisa que o membro será desconectado). Decisões do usuário: não forçar troca no próximo login (sem flag/migration) e admin digita a nova senha. Testes: **+7** em `OrganizacaoServiceTest` (26: sucesso com encode/bump, 403 não-membro, 409 não-admin, 404 alvo não-membro, 409 criador, 409 self, 400 senha igual) e **+3** em `OrganizacoesIntegrationTest` (11: login com a nova senha + 401 com a antiga + token antigo invalidado, 403 operador, 409 criador) → backend **182/182**, BUILD SUCCESS; cobertura **84,7% instruções / 70,1% ramos**; frontend oxlint **0** + Vitest **51/51** + `tsc -b && vite build` OK. Docs: README (badge `84.7%`, linha do endpoint e 182 testes) e `testes.md`. Commits sugeridos ao usuário: `feat(api): allow group admins to reset member passwords`, `feat(ui): add reset password action for group members` e `docs: document admin member password reset`. |
| 2026-09-17 | opencode/big-pickle | **Plano A — smoke E2E cobrindo os dois fluxos de redefinição de senha + resíduos de migration nos docs**. `scripts/test-api.sh` ganhou a seção 12 "Redefinição de senha" com **12 verificações**: reset **público** (`POST /api/auth/redefinir-senha`) — senha atual incorreta 400, e-mail inexistente 400, sucesso 200, login com a nova senha 200, senha antiga 401 e token anterior revogado 401 (bump de `versao_token`); e reset pelo **ADMIN do grupo** (`PUT /api/organizacoes/{id}/membros/{usuarioId}/senha`) — OPERADOR 403, ADMIN contra si mesmo 409, alvo não-membro 404, sucesso 204, login com a nova senha 200 e sessão anterior do membro revogada 401. **Achado**: o total documentado (52) já estava desatualizado — baseline real medido = 47; total agora **59**, conferido rodando contra a API real (**59 OK / 0 falhas**). A seção reaproveita usuários já criados para caber nos limites de rate limit (3 redefinições ≤ 5/900s, 9 logins ≤ 10/60s, 4 registros ≤ 5/300s). Docs: refs Flyway **V1–V8 → V1–V9** (README/`backup.md`/`deploy.md`) e exemplos de nova migration `V9__...` → `V10__...` (`deploy.md`); contagens 52 → 59 e descrição do smoke atualizada (README/`testes.md`/`deploy.md`). Sem alteração de código de produção. Commits sugeridos ao usuário: `test(e2e): cover password reset flows in smoke script` e `docs: refresh migration range to V9 and smoke count`. |
| 2026-09-20 | opencode/big-pickle | **Rodada de 8 commits (todos pushados, topo `5f89d1e`), validação e fechamento desta sessão**. **Migrations**: `V10__convites universais` (código de acesso unificado para convite universal) e `V11__empenho com número sequenciado` (coluna de numeração sequencial por organização), comutadas e aplicadas no banco. **Backend**: licitação com vencedor dentro do fluxo multitenancy, modalidades das Leis 8.666/93 e 14.133/21 e validação de datas; `EmpenhoServiceTest`/integrações reconciliados para os fluxos de vencedor e convidadas. **Frontend**: máscaras de CNPJ, moeda e edital + validação de campos obrigatórios; páginas de UI para **aceitar/recusar convite universal** e seletor de vencedor com autocomplete; alinhamento visual dos formulários. **Smoke E2E reescrito para 86 verificações** (`scripts/test-api.sh`: 82 `assert_status` + 4 incrementos diretos) — validado contra a API real nesta sessão (**86/86 OK**). **Docs/diagrama reconciliados**: README, `testes.md`, `deploy.md`, `modalidades.md` e `modelo-dados.mermaid` (convite universal + empenho sequenciado, migrations V1–V11); GIF demo regravado (`docs/img/demo.gif`, 11 cenas, 3,16 MB). |
